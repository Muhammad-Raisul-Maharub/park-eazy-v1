        ...
        <div className="h-full w-full flex bg-slate-100 dark:bg-slate-900 relative">
            <div className="flex-1 relative h-full w-full overflow-hidden">
                ...
                <MapContainer center={chittagongCoords} zoom={defaultZoom} maxZoom={18} className="h-full w-full focus:outline-none bg-slate-100 dark:bg-slate-900" zoomControl={false} zoomAnimation={true} markerZoomAnimation={true} scrollWheelZoom={true}>
                    ...
                    {userLocation && <Circle center={userLocation} radius={Math.min(accuracy || 20, 20)} pathOptions={{ color: '#3b82f6', fillColor: '#3b82f6', fillOpacity: 0.15, weight: 1.5, dashArray: '5, 5', stroke: true }} />}
                ...
                <div ref={searchContainerRef} className="absolute top-4 left-1/2 -translate-x-1/2 z-[1200] w-[92%] sm:w-[95%] max-w-lg space-y-2 map-controls-container" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="absolute top-20 right-4 sm:right-auto sm:left-4 z-[1100]">
                        <button onClick={() => window.location.reload()} className="p-2 bg-white/90 dark:bg-slate-800/90 backdrop-blur text-slate-500 hover:text-primary rounded-xl shadow-lg border border-white/50 dark:border-slate-700 transition-all hover:scale-105" title="Reload Map">
