...
const UserSettingsPage: React.FC = () => {
    ...
    const { addToast } = useContext(LogContext);
    ...
    const TabButton: React.FC<{ tabName: 'profile' | 'activity' | 'payment', label: string, icon: React.ElementType }> = ({ tabName, label, icon: Icon }) => (
        <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 p-3 text-sm font-semibold border-b-2 transition-colors duration-300 ${activeTab === tabName
