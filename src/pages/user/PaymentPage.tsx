...
const PaymentPage: React.FC = () => {
    ...
    const isFormValid = useMemo(() => {
        if (selectedSavedMethodId) return true;
        if (selectedMethod === PaymentMethod.CARD) {
    ...
    }, [selectedMethod, cardDetails, mobileWalletNumber, selectedSavedMethodId]);
    ...
    const handlePayment = async () => {
        ...
        try {
            if (saveMethod && !selectedSavedMethodId) {
                if (selectedMethod === PaymentMethod.CARD) {
                    addMethod({
                        id: `card-${Date.now()}`,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
                ...
                <div className="lg:col-span-7 space-y-6">
                    <div className="bg-slate-900/60 border border-slate-800 backdrop-blur-sm p-6 sm:p-8 rounded-3xl">
                            ...
                            {paymentMethods.map(method => (
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        setSelectedSavedMethodId(null); // Reset saved selection
                                        setErrors({});
                                    }}
                        ...
                        {paymentContext.savedMethods.filter(m => m.type === selectedMethod).length > 0 && (
                            <div className="mb-6">
                                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Saved {selectedMethod}s</h3>
                                <div className="space-y-3">
                                    {paymentContext.savedMethods.filter(m => m.type === selectedMethod).map(method => (
                                        <div
                                            key={method.id}
                                            onClick={() => setSelectedSavedMethodId(method.id)}
                                            className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${selectedSavedMethodId === method.id
                                                ? 'bg-fuchsia-500/10 border-fuchsia-500 ring-1 ring-fuchsia-500/50'
                                                : 'bg-slate-900/50 border-slate-700 hover:border-slate-600'
                                                }`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`p-2 rounded-full ${selectedSavedMethodId === method.id ? 'bg-fuchsia-500 text-white' : 'bg-slate-800 text-slate-400'}`}>
                                                    <CheckCircle className={`w-4 h-4 ${selectedSavedMethodId === method.id ? 'opacity-100' : 'opacity-0'}`} />
                                                </div>
                                                <div>
                                                    {method.type === PaymentMethod.CARD ? (
                                                        <>
                                                            <p className="font-bold text-slate-200">{(method as SavedCard).cardholderName}</p>
                                                            <p className="text-xs text-slate-500 font-mono">**** **** **** {(method as SavedCard).last4} | Exp: {(method as SavedCard).expiryDate}</p>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <p className="font-bold text-slate-200">{method.type} Wallet</p>
                                                            <p className="text-xs text-slate-500 font-mono">{(method as SavedMobileWallet).accountNumber}</p>
                                                        </>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div
                                        ...
                                        className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${selectedSavedMethodId === null
                                            ? 'bg-slate-800/50 border-fuchsia-500 text-fuchsia-400'
                                            : 'bg-transparent border-dashed border-slate-700 text-slate-500 hover:text-slate-300 hover:border-slate-600'
                                            }`}
                                    >
                                        <PlusCircle className="w-5 h-5" />
                                        <span className="font-bold text-sm">Use a new {selectedMethod}</span>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div>
                            {selectedSavedMethodId ? null : (
                                selectedMethod === PaymentMethod.CARD ? (
                                    <div className="rounded-2xl border-2 border-fuchsia-500/50 bg-slate-900/50 p-1 overflow-hidden animate-fadeIn">
