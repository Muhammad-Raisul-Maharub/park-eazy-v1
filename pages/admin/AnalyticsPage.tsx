...
const DonutChart: React.FC<{ data: { status: string, count: number, color: string }[], total: number }> = ({ data, total }) => {
    if (total === 0) {
        return <div className="flex items-center justify-center h-48 w-48 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500">No Data</div>;
    }
...
const AdminAnalyticsPage: React.FC = () => {
    ...
    const slotStatusData = [
        { status: 'Available', count: availableSlots, color: 'bg-green-500', icon: CheckCircle, textColor: 'text-green-500' },
        { status: 'Reserved', count: reservedSlots, color: 'bg-amber-500', icon: PauseCircle, textColor: 'text-amber-500' },
        { status: 'Occupied', count: occupiedSlots, color: 'bg-red-500', icon: XCircle, textColor: 'text-red-500' },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard icon={TrendingUp} title="Occupancy Trend" value="+2.5%" change={2.5} changeType="increase" />
                <StatCard icon={CircleDollarSign} title="Avg. Revenue/Slot" value={formatCurrency(350.75)} change={5} changeType="increase" />
                <StatCard icon={Clock} title="Peak Hours" value="5 PM - 8 PM" />
                <StatCard icon={ParkingSquare} title="Most Used Slot Type" value="SUV" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Revenue by Slot Type (Monthly)</h2>
                    <div className="space-y-4">
                        {revenueData.map(item => (
                            <div key={item.type} className="flex items-center">
                                <span className="w-20 text-sm font-medium text-slate-600 dark:text-slate-400">{item.type}</span>
                                <div className="flex-1 bg-slate-200 dark:bg-slate-700 rounded-full h-6">
                                    ...
                                    >
                                    <span className="text-xs font-bold text-white">{formatCurrency(item.revenue)}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <Card>
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Live Slot Status Distribution</h2>
                    <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-8">
                        ...
                        <ul className="space-y-3">
                            {slotStatusData.map(d => {
                                ...
                                return (
                                    <li key={d.status} className="flex items-center text-sm">
                                        <Icon className={`w-4 h-4 mr-2 ${d.textColor}`} />
                                        <span className="font-medium text-slate-700 dark:text-slate-300 w-20">{d.status}</span>
                                        <span className="font-semibold text-slate-800 dark:text-slate-100">{d.count}</span>
                                        <span className="ml-auto font-mono text-slate-500 dark:text-slate-400">{percentage}%</span>
                                    </li>
                                )
                            })}
                        </ul>
